...
function Rules({ rules, browsers, setRules }: { rules: Rule[]; browsers: BrowserInfo[]; setRules: (r: Rule[]) => void }) { return <div className="content"><Section title="Ссылки сами найдут нужный браузер" hint="Создавайте правила для сайтов, рабочих сервисов и отдельных профилей."><div className={`rules ${rules.length === 1 ? 'single' : ''}`}>{rules.map(rule => <div className="rule" key={rule.id}><Toggle value={rule.enabled} onChange={v => setRules(rules.map(x => x.id === rule.id ? {...x, enabled: v} : x))}/><div className="rule-match"><Globe2 size={17}/><div><span>Адрес содержит</span><input value={rule.match} onChange={e => setRules(rules.map(x => x.id === rule.id ? {...x, match: e.target.value} : x))}/></div></div><span className="arrow">→</span><select value={rule.browser} onChange={e => setRules(rules.map(x => x.id === rule.id ? {...x, browser: e.target.value} : x))}>{browsers.map(b => <option key={b.id} value={b.id}>{b.name}</option>)}</select><button className="delete" onClick={() => setRules(rules.filter(x => x.id !== rule.id))}><Trash2 size={16}/></button></div>)}</div></Section></div> }
