...
export default function App() {
  ...
  const closePicker = () => { setPickerOpen(false); if (externalPicker) window.picklink?.close() }
  const chooseBrowser = async (browser: BrowserInfo, profileOverride?: BrowserProfile) => { const profile = profileOverride ?? browser.profiles?.find(item => item.id === selectedProfiles[browser.id]); await window.picklink?.openUrl(browser, testUrl, profile); closePicker(); notify(`Открываю в ${browser.name}${profile ? ` · ${profile.name}` : ''}`) }
  useEffect(() => {
  ...
  return <div className={`app theme-${activeTheme}${externalPicker ? ' compact-picker' : ''}`}>
    ...
    {pickerOpen && <div className="overlay" onMouseDown={closePicker}><div className="picker" onMouseDown={e => e.stopPropagation()}><div className="picker-top"><div className="site-icon"><Globe2 size={20}/></div><div><span>Открыть ссылку</span><strong>{testUrl.replace(/^https?:\/\//, '')}</strong></div><button onClick={closePicker}><X size={17}/></button></div><div className="browser-grid">{choices.map(({ id, browser:b, profile, key }) => <button key={id} onClick={() => chooseBrowser(b, profile)}><BrowserMark browser={b} size={46}/><span>{b.name.replace('Microsoft ', '').replace('Google ', '').replace('Mozilla ', '')}</span>{profile ? <small>{profile.name}</small> : null}<kbd>{key}</kbd></button>)}</div><div className="picker-rule"><button onClick={() => { setRules([...rules, { id: Date.now(), match: new URL(testUrl).hostname, browser: browsers[0]?.id ?? 'edge', enabled: true }]); notify('Правило добавлено'); closePicker() }}><Plus size={15}/>Всегда открывать этот сайт в выбранном браузере</button></div></div></div>}
...
function Rules({ rules, browsers, setRules }: { rules: Rule[]; browsers: BrowserInfo[]; setRules: (r: Rule[]) => void }) { return <div className="content"><Section title="Ссылки сами найдут нужный браузер" hint="Создавайте правила для сайтов, рабочих сервисов и отдельных профилей."><div className={`rules ${rules.length === 1 ? 'single' : ''}`}>{rules.map(rule => <div className="rule" key={rule.id}><Toggle value={rule.enabled} onChange={v => setRules(rules.map(x => x.id === rule.id ? {...x, enabled: v} : x))}/><div className="rule-match"><Globe2 size={17}/><div><span>Адрес содержит</span><input value={rule.match} title={rule.match} onChange={e => setRules(rules.map(x => x.id === rule.id ? {...x, match: e.target.value} : x))}/></div></div><span className="arrow">→</span><select value={rule.browser} onChange={e => setRules(rules.map(x => x.id === rule.id ? {...x, browser: e.target.value} : x))}>{browsers.map(b => <option key={b.id} value={b.id}>{b.name}</option>)}</select><button className="delete" onClick={() => setRules(rules.filter(x => x.id !== rule.id))}><Trash2 size={16}/></button></div>)}</div></Section></div> }
